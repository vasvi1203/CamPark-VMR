...
def successPage(request):
    ...
    # stream = ffmpeg.output(stream, temp[:-5] + ".mp4")
    # ffmpeg.run(stream, overwrite_output = True)
    
    # os.remove(temp)
        
    return render(request, 'main_ui/success.html', {'title': "Query Page", 'query': query, 'video': video.videoFile.url[:-4] + "trimmed1.mp4", 'start_pred' : span_pred[0], 'end_pred': span_pred[1],'start_gt' : span_gt[0], 'end_gt': span_gt[1]})